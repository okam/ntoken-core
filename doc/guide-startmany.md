...
2. [Sending 1000 NTK to new wallet addresses.](#option2)
...
Just like setting up a standard MN. Send exactly 1000 NTK to each new address created above.
...
[From the ntoken github repo](https://github.com/ntoken/ntoken/blob/master/doc/masternode_conf.md)
...
## What about the ntoken.conf file?

If you are using a `masternode.conf` file you no longer need the `ntoken.conf` file. The exception is if you need custom settings (_thanks oblox_). In that case you **must** remove `masternode=1` from local `ntoken.conf` file. This option should be used only to start local Hot masternode now.
